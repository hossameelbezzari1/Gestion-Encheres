    ...
    return (
        <>
                    ...
                    { key: 'produits_count', label: 'Produits' },
                    {
                        ...
                        render: (row) => (
                            <div className="flex gap-1">
                                <Link href={route('categories.edit', row.id)} className="rounded p-2 hover:bg-slate-100">
            ...
            <ConfirmDialog
                ...
                onConfirm={() => router.delete(route('categories.destroy', selected.id), { onFinish: () => setSelected(null) })}
                description={`Supprimer la catégorie « ${selected?.nom || ''} » ?`}
            />
        </>
    );
}
